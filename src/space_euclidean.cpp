...
EuclideanSpace::EuclideanSpace( std::vector<std::string> names, std::vector<value> bandwidth, std::vector<value> location) :
EuclideanSpace::EuclideanSpace( names, GaussianKernel(), bandwidth, location ) {}

EuclideanSpace::EuclideanSpace( std::vector<std::string> names, const Kernel & k, std::vector<value> bandwidth, std::vector<value> location) :
SpaceBase<EuclideanSpace>( "euclidean", make_spec( names, k), make_kernel( names.size(), bandwidth, location, k ) ), kernel_( k.clone() ) {}

SpaceSpecification EuclideanSpace::make_spec( std::vector<std::string> names, const Kernel & ktype ) {
...
Component EuclideanSpace::make_kernel(unsigned int n, std::vector<value> bw, std::vector<value> loc, const Kernel & ktype ) const {
    Component k;
...
EuclideanSpace * EuclideanSpace::fromYAML( const YAML::Node & node ) {
    ...
    EuclideanSpace * p = new EuclideanSpace( dims, *k );

    return p;
}

YAML::Node EuclideanSpace::asYAML() const {
    YAML::Node node;
...
value EuclideanSpace::compute_scale_factor( std::vector<bool>::const_iterator selection, value * bw, bool log) const {
    
    return kernel_->scale_factor( ndim(), bw, log, selection );
}


value EuclideanSpace::mahalanobis_distance_squared( const value * refloc, const value * refbw, const value * targetloc, value threshold) const {
...
void EuclideanSpace::merge( value w1, value * loc1, value * bw1, value w2, const value * loc2, const value * bw2 ) const {
...
value EuclideanSpace::probability( const value * loc, const value * bw, const value * point ) const {
    
    return kernel_->probability( ndim(), loc, bw, point );
}

void EuclideanSpace::probability( const value * loc, const value * bw, const value * points, unsigned int n, value * result ) const {
    for (unsigned int k=0; k<n; ++k) {
        *result++ = kernel_->probability(1, loc, bw, points++);
    }
}

value EuclideanSpace::log_probability( const value * loc, const value * bw, const value * point ) const {
    
    return kernel_->log_probability( ndim(), loc, bw, point );
}

void EuclideanSpace::log_probability( const value * loc, const value * bw, const value * points, unsigned int n, value * result ) const {
    for (unsigned int k=0; k<n; ++k) {
        *result++ = kernel_->log_probability(1, loc, bw, points++);
    }
}

value EuclideanSpace::partial_logp( const value * loc, const value * bw, const value * point, std::vector<bool>::const_iterator selection ) const {
    
    return kernel_->partial_logp( ndim(), loc, bw, point, selection );
}


VectorGrid * EuclideanSpace::grid( const std::vector<std::vector<value>> & vectors, const std::vector<bool> & valid, const std::vector<bool> & selection ) const {
    ...
    if (nvectors==ndim()) {
        ...
        if (nselected!=nvectors) {
            throw std::runtime_error("Mismatch between number of grid vectors and number of selected dimensions");
        }
    }
