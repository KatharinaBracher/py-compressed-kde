...
public:
    EncodedSpace( std::string name, std::vector<value> & lut, value bandwidth = DEFAULT_ENCODED_BANDWIDTH, unsigned int index = DEFAULT_ENCODED_INDEX);
    EncodedSpace( std::string name, std::vector<value> & lut, const Kernel & k, value bandwidth = DEFAULT_ENCODED_BANDWIDTH, unsigned int index = DEFAULT_ENCODED_INDEX);
    
    EncodedSpace( const EncodedSpace & other ) : SpaceBase<EncodedSpace>(other), nlut_(other.nlut_), lut_(other.lut_), kernel_(other.kernel_->clone()) {}
    
    SpaceSpecification make_spec( std::string name, std::vector<value> & lut, const Kernel & k );
    Component make_kernel( value bw, unsigned int idx, const Kernel & k) const;
    
    static EncodedSpace * fromYAML( const YAML::Node & node );
